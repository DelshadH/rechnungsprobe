...
from rechnungsprobe.target import ContainerTarget, LocalTarget, run_local_target
...
def test_replay_command_reexecutes_the_verified_capsule_target(
    tmp_path: Path,
    capsys: pytest.CaptureFixture[str],
) -> None:
    invoice_xml = serialize_invoice(parse_invoice(bundled_seed_path()))
    ...
    assert main(["replay", str(capsule), "--allow-local-target"]) == 0
    assert json.loads(capsys.readouterr().out) == {
        "case_id": "synthetic-replay",
        "predicate": "crash-or-nonzero",
        "status": "reproduced",
...
def test_replay_command_requires_explicit_consent_for_a_local_target(
    ...
    assert json.loads(capsys.readouterr().err) == {
        "error": "local capsule targets require --allow-local-target",
        "status": "error",
    }
...
def test_replay_preflights_target_digest_before_local_execution(
    ...
    assert main(
        ["replay", str(capsule), "--allow-local-target"]
    ) == 2
    assert json.loads(capsys.readouterr().err) == {
